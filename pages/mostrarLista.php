...
$sqlLista = "SELECT p.idPlay, p.imgPlay, p.nomPlay AS nombre, COUNT(l.idPlay) AS cantidad FROM playlist p LEFT JOIN lista l ON p.idPlay = l.idPlay WHERE p.idUser = $userId GROUP BY p.idPlay, p.nomPlay";
...
if ($resultCancion->num_rows > 0) {
   // $rowCancion = $resultCancion->fetch_assoc();
    ...
    echo '<div class="card-header" data-valor="' . htmlspecialchars($rowCancion["idPlay"]) . '" onclick="registrarCancion(this)" >';
    echo'<div class="card-body">';
    echo '<img src="' . $rowCancion["imgPlay"] .'">';
    echo '</div>';
    echo '<div class="card-body">';
